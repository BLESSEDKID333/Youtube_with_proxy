...
- (NSString *)currentCombinedParams {
    ...
    if (seg == 1) typeParam = @"EgIQAQ==";      // Videos only
    else if (seg == 2) typeParam = @"EgIQAg=="; // Channels only

    if (typeParam) return typeParam;
    return self.selectedSortParam;
}

- (void)performSearch {
    if (self.searchBar.text.length == 0) return;
    [self.loadingIndicator startAnimating];
    [self.apiManager searchFromWeb:self.searchBar.text params:[self currentCombinedParams]];
}
