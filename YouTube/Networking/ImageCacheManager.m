...
- (void)loadImageFromURL:(NSString *)urlString completion:(void (^)(UIImage *image))completion {
    ...
    dispatch_async(self.imageQueue, ^{
        ...
        // Force-decode the JPEG here on the background queue. Otherwise UIKit
        // decompresses lazily on the main thread at first draw, which causes
        // visible scroll stutter on the iPhone 5's single/dual-core CPU.
        image = [self decodedImageFromImage:image];
