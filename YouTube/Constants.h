...
#define VPS_HOSTS_LIST_KEY      @"vps_hosts_list"
#define VPS_ACTIVE_INDEX_KEY    @"vps_active_index"
#define VPS_PROXY_DEFAULT       @"http://192.144.13.102"

// ==================== iOS 7 STYLE TOGGLE ====================
#define IOS7_STYLE_KEY          @"ios7_style_enabled"

static inline BOOL IOS7StyleEnabled(void) {
    // Off by default — classic iOS 6 look unless the user opts in.
    return [[NSUserDefaults standardUserDefaults] boolForKey:IOS7_STYLE_KEY];
}

static inline BOOL VPSBypassEnabled(void) {
    return ([[NSUserDefaults standardUserDefaults] objectForKey:VPS_BYPASS_KEY] == nil)
        ? YES : [[NSUserDefaults standardUserDefaults] boolForKey:VPS_BYPASS_KEY];
}

static inline NSString * VPSProxyBase(void) {
    NSString *saved = [[NSUserDefaults standardUserDefaults] stringForKey:VPS_PROXY_KEY];
    if (saved && saved.length > 5) return saved;

    NSArray *hosts = [[NSUserDefaults standardUserDefaults] arrayForKey:VPS_HOSTS_LIST_KEY];
    NSInteger idx = [[NSUserDefaults standardUserDefaults] integerForKey:VPS_ACTIVE_INDEX_KEY];
    if (hosts && [hosts count] > 0 && idx >= 0 && idx < [hosts count]) {
        NSString *h = [hosts objectAtIndex:idx];
...
// InnerTube Endpoints (VPS proxy vs Direct Native)
...
// ==================== NATIVE (NO-VPS) PLAYER ====================
#define INNERTUBE_ANDROID_CLIENT_NAME        @"ANDROID"
#define INNERTUBE_ANDROID_CLIENT_NAME_HEADER @"3"
#define INNERTUBE_ANDROID_CLIENT_VERSION     @"21.03.36"
#define INNERTUBE_ANDROID_OS_VERSION         @"16"
#define INNERTUBE_ANDROID_SDK_VERSION        36
#define INNERTUBE_ANDROID_USER_AGENT         @"com.google.android.youtube/21.03.36 (Linux; U; Android 15; US) gzip"
#define DIRECT_INNERTUBE_PLAYER @"https://www.youtube.com/youtubei/v1/player?prettyPrint=false"
